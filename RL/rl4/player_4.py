...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        if np.random.random() < epsilon:
            action = np.random.choice(all_actions)  # Randomly choosing action
        else:
            action = np.nanargmax(Q[state]) # Taking the greedy action (best action)
        # ADD YOUR CODE SNIPPET BETWEEN EX 4.1
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        diff_epsilon = (self.final_p - self.initial_p)
        annealed_epsilon = self.initial_p + diff_epsilon * (t / self.schedule_timesteps)
